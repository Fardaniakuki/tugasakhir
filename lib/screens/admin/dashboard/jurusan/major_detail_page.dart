...
class _MajorDetailPageState extends State<MajorDetailPage> {
  ...
  Future<void> fetchMajorDetail() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorDialog('Gagal mengambil data jurusan');
      }
    } catch (_) {
      setState(() => isLoading = false);
  ...
  Future<void> deleteMajor() async {
    try {
      ...
      final int? majorId = majorData?['id'];
      if (majorId == null) {
        _showErrorDialog('ID jurusan tidak ditemukan');
        return;
      }

      final response = await http.delete(
        Uri.parse('${dotenv.env['API_BASE_URL']}/api/jurusan/$majorId'),
        headers: {
          'Accept': 'application/json',
            ...
            Column(
              children: classes.map((classData) {
                return Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.class_rounded, color: Colors.grey, size: 20),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
