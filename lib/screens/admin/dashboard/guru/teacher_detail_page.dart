...
class _TeacherDetailPageState extends State<TeacherDetailPage> {
  ...
  Future<void> fetchTeacherDetail() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showErrorDialog('Gagal mengambil data guru');
      }
    } catch (_) {
      setState(() => isLoading = false);
  ...
  Future<void> deleteTeacher() async {
    try {
      ...
      final int? teacherId = teacherData?['id'];
      if (teacherId == null) {
        _showErrorDialog('ID guru tidak ditemukan');
        return;
      }

      final response = await http.delete(
        Uri.parse('${dotenv.env['API_BASE_URL']}/api/guru/$teacherId'),
        headers: {
          'Accept': 'application/json',
  ...
  Widget _buildStatusBadge(String label, bool? value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      decoration: BoxDecoration(
        color: value == true ? _successColor.withValues(alpha:0.1) : Colors.grey.withValues(alpha:0.1),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: value == true ? _successColor : Colors.grey,
          width: 1,
        ),
      ),
      child: Row(
          ...
          Expanded(
            child: Text(
              ...
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w600,
                color: value == true ? _successColor : Colors.grey[700],
              ),
            ),
          ),
        ],
                                          ...
                                          Padding(
                                            ...
                                            child: Column(
                                              children: [
                                                ...
                                                _buildStatusBadge('Koordinator', teacherData?['is_koordinator']),
                                                const SizedBox(height: 10),
                                                _buildStatusBadge('Pembimbing', teacherData?['is_pembimbing']),
                                                const SizedBox(height: 10),
