...
class _UploadPageState extends State<UploadPage> {
  ...
  // ========== METHOD FORMAT TANGGAL INDONESIA ==========
  String _formatTanggalIndonesia(DateTime date) {
    final hari = _getIndonesianDayName(DateFormat('EEEE').format(date));
    final bulan = _getIndonesianMonthName(DateFormat('MMMM').format(date));
    return '$hari, ${date.day} $bulan ${date.year}';
  }

  String _formatTanggalRangeIndonesia(DateTime start, DateTime end) {
    if (start.year == end.year &&
        start.month == end.month &&
        start.day == end.day) {
      return _formatTanggalIndonesia(start);
    }

    final startHari = _getIndonesianDayName(DateFormat('EEEE').format(start));
    final endHari = _getIndonesianDayName(DateFormat('EEEE').format(end));
    final startBulan =
        _getIndonesianMonthName(DateFormat('MMMM').format(start));
    final endBulan = _getIndonesianMonthName(DateFormat('MMMM').format(end));

    if (start.year == end.year && start.month == end.month) {
      return '$startHari, ${start.day} - $endHari, ${end.day} $startBulan ${start.year}';
    } else if (start.year == end.year) {
      return '$startHari, ${start.day} $startBulan - $endHari, ${end.day} $endBulan ${start.year}';
    } else {
      return '$startHari, ${start.day} $startBulan ${start.year} - $endHari, ${end.day} $endBulan ${end.year}';
    }
  }
  ...
  void _generateCalendar() {
    ...
    _currentMonth =
        '${_getIndonesianMonthName(englishMonth)} ${_currentDate.year}';
    _calendarDays = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _activeKegiatan =
              kegiatanList.where((k) => k.status == 'active').toList();
  ...
  bool _hasEvent(DateTime date) {
    final hasJadwal =
        _jadwalEvents.containsKey(DateTime(date.year, date.month, date.day));
    final hasRiwayat =
        _events.containsKey(DateTime(date.year, date.month, date.day));
  ...
  Future<void> _startUploadProcess({
    ...
    if (_isTaskAlreadyUploaded(industriId, kegiatanId)) {
      _showSnackBar('⚠️ Task ini sudah pernah diunggah!', isError: true);
  ...
  bool _isTaskAlreadyUploaded(int industriId, int kegiatanId) {
    ...
    if (_uploadedFiles.containsKey(taskKey) &&
        _uploadedFiles[taskKey]!.isNotEmpty) {
      return true;
    }
  ...
  Future<void> _showAlreadyUploadedDialog(
      int industriId, int kegiatanId) async {
    final taskKey = '${industriId}_$kegiatanId';
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const Text(
                'Task ini sudah pernah diunggah sebelumnya.',
              ...
              if (imageUrls.isNotEmpty) ...[
                const SizedBox(height: 16),
                const Text(
                  'Bukti foto yang sudah diunggah:',
  ...
  Future<void> _showImageSourceDialog(
      int industriId, int kegiatanId, dynamic kegiatan) async {
                ...
                ElevatedButton(
                  onPressed: isUploading
                      ...
                      : () async {
                          ...
                          try {
                            final url =
                                await _uploadSingleImage(_selectedImages.first);
                            ...
                            if (url != null && url.isNotEmpty) {
                              ...
                              if (mounted) {
                                _showSnackBar('❌ Gagal unggah gambar',
                                    isError: true);
                              }
                            }
                          } catch (e) {
        ...
        if (mounted) {
          try {
            ...
            final errorMsg = errorData['message'] ??
                errorData['error'] ??
                'Unggah gagal: ${response.statusCode}';
            _showSnackBar(errorMsg, isError: true);
          } catch (e) {
            _showSnackBar('Unggah gagal: ${response.statusCode}',
                isError: true);
          }
        }
        return null;
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (mounted) {
          try {
            ...
            final errorMsg = errorData['message'] ??
                errorData['error'] ??
                'Submit gagal: ${response.statusCode}';
            _showSnackBar(errorMsg, isError: true);
          } catch (e) {
            _showSnackBar('Submit gagal: ${response.statusCode}',
                isError: true);
          }
        }
        return false;
...
// ========== METHOD UPDATE REALISASI (PUT) ==========
  Future<bool> _updateRealisasiData({
    required int realisasiId,
    required List<String> imageUrls,
  }) async {
    try {
      if (_accessToken == null) {
      ...
      final updateUrl =
          Uri.parse('$baseUrl/api/realisasi-kegiatan/$realisasiId');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        print('✅ Update berhasil untuk realisasi ID: $realisasiId');
        return true;
      } else {
        ...
        if (mounted) {
          try {
            ...
            final errorMsg = errorData['message'] ??
                errorData['error'] ??
                'Update gagal: ${response.statusCode}';
            _showSnackBar(errorMsg, isError: true);
          } catch (e) {
            _showSnackBar('Update gagal: ${response.statusCode}',
                isError: true);
          }
        }
        return false;
      ...
      if (mounted) {
        _showSnackBar('Update error: ${e.toString()}', isError: true);
      }
      return false;
    }
  }

// ========== DIALOG UPDATE REALISASI (SEDERHANA) ==========
  Future<void> _showUpdateRealisasiDialog(Map<String, dynamic> riwayat) async {
    final realisasiId = riwayat['id'];

    // Ambil URL gambar yang sudah ada
    List<String> existingImageUrls = [];
    try {
      if (riwayat['bukti_foto_urls'] is List) {
        existingImageUrls = List<String>.from(riwayat['bukti_foto_urls']);
      } else if (riwayat['bukti_foto'] is String) {
        existingImageUrls = [riwayat['bukti_foto']];
      } else if (riwayat['bukti_foto'] is List) {
        existingImageUrls = List<String>.from(riwayat['bukti_foto']);
      }
    } catch (e) {
                              ...
                              Center(
                                child: Container(
                                  ...
                                  child: ClipRRect(
                                    ...
                                    child: Image.file(
                                      File(newSelectedImages.first.path),
                                      fit: BoxFit.cover,
                                    ),
                                  ),
                                ),
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const Expanded(
                                    child: Text(
                                      'Foto baru akan mengganti semua foto yang sudah ada',
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: Colors.blue,
                                      ),
                            ...
                            onPressed: isUploading
                                ...
                                : () async {
                                    if (newSelectedImages.isEmpty) {
                                      setDialogState(() {
                                        status = '❌ Pilih foto terlebih dahulu';
                                      });
                                      return;
                                    }
...
// ========== DIALOG UPDATE REALISASI (FIXED VERSION) ==========
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Text(
                                '${_getEventsForDay(_selectedDate!).where((e) => e['type'] == 'jadwal').length} Jadwal, ${_getEventsForDay(_selectedDate!).where((e) => e['type'] == 'riwayat').length} Unggah',
                                style: const TextStyle(
                                  fontSize: 12,
                                  color: Color(0xFF666666),
                                ),
                ...
                child: Column(
                  children: [
                    if (_industriList.isEmpty)
                      ...
                      ..._industriList.map(
                          (industriData) => _buildIndustriCard(industriData)),
                  ],
                ),
              ),
            ...
            child: Column(
              children: [
                ...
                ...tasks.map((taskData) {
                  ...
                  final isUploaded =
                      _isTaskAlreadyUploaded(industriId, kegiatanId);
                      ...
                      children: [
                        ...
                        Expanded(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                // MODIFIKASI: Ubah 'Pembekalan' menjadi 'Pengantaran'
                                kegiatan['jenis'] == 'Pembekalan'
                                    ? 'Pengantaran'
          ...
          children: [
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 8),
                    // Edit button - FIXED WITH CONTAINER AND CONSTRAINTS
                    Container(
                      width: 36,
                      height: 36,
                      decoration: BoxDecoration(
                      ...
                      child: IconButton(
                        key: ValueKey('edit_button_${riwayat['id']}'),
                        icon: Icon(
                          Icons.edit,
                          color: widget.primaryColor,
                          size: 18,
                        ),
                        onPressed: () => _showUpdateRealisasiDialog(riwayat),
                        tooltip: 'Update Bukti',
            ...
            Row(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Text(
                      _formatTanggalIndonesia(
                          DateTime.parse(riwayat['tanggal_realisasi'])),
                      style: const TextStyle(
                        color: Color(0xFF666666),
                        fontSize: 13,
                      ),
                ...
                Text(
                  _formatTanggalSingkat(DateTime.parse(
